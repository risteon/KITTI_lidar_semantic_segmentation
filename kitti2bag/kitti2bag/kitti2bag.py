...
def save_velo_data(bag, kitti, velo_frame_id, topic, velodyne_folder='velodyne_points'):
    print("Exporting velodyne data")
    # velo_path = os.path.join(kitti.data_path, velodyne_folder)
    velo_path = velodyne_folder
...
def save_gps_fix_data(bag, kitti, gps_frame_id, topic):
    print("Exporting GPS FIX data on topic {}".format(topic))
    for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
...
def run_kitti2bag():
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        try:
            ...
            imu_topic = 'imu'                 # < topic for google cartographer
            gps_fix_topic = 'fix'             # < topic for google cartographer
            gps_vel_topic = '/kitti/oxts/gps/vel'
            velo_frame_id = 'velo_link'
            velo_topic = 'points2'            # < topic for google cartographer
            ...
            save_velo_data(bag, kitti, velo_frame_id, velo_topic,
                           velodyne_folder=os.path.join(args.dir2,
                               'velodyne_points_partitioned_truncated'))
        ...
        bag = rosbag.Bag("kitti_data_odometry_{}_sequence_{}_part2.bag".format(args.kitti_type[5:],args.sequence), 'w', compression=compression)
